...
# When E2B_LOCAL_MODE=true, SDK connects to local envd via Host IP
LOCAL_MODE_ENV = "E2B_LOCAL_MODE"
LOCAL_ENVD_URL_TEMPLATE_ENV = "E2B_LOCAL_ENVD_URL_TEMPLATE"
# Default template: http://{host_ip}:{port} where host_ip comes from API response
DEFAULT_LOCAL_ENVD_URL_TEMPLATE = "http://{host_ip}:{port}"
...
class ConnectionConfig:
    ...
    For local E2B deployments, set E2B_LOCAL_MODE=true and optionally
    E2B_LOCAL_ENVD_URL_TEMPLATE to customize the envd URL format.

    Example for local deployment:
        export E2B_LOCAL_MODE=true
        export E2B_API_URL=http://localhost:3000
        export E2B_API_KEY=your_local_api_key
    """
    ...
    def _local_mode():
        ...
        return os.getenv(LOCAL_MODE_ENV, "false").lower() == "true"

    @staticmethod
    def _local_envd_url_template():
        """Get the URL template for local envd connections."""
        return os.getenv(LOCAL_ENVD_URL_TEMPLATE_ENV, DEFAULT_LOCAL_ENVD_URL_TEMPLATE)
    ...
    def __init__(
        ...
        local_envd_url_template: Optional[str] = None,
    ):
        self.domain = domain or ConnectionConfig._domain()
        self.debug = debug or ConnectionConfig._debug()
        self.local_mode = local_mode if local_mode is not None else ConnectionConfig._local_mode()
        self.local_envd_url_template = local_envd_url_template or ConnectionConfig._local_envd_url_template()
        ...
        self.api_url = (
            ...
            or ("http://localhost:3000" if (self.debug or self.local_mode) else f"https://api.{self.domain}")
        )
    ...
    def get_sandbox_url(self, sandbox_id: str, sandbox_domain: str, host_ip: Optional[str] = None) -> str:
        """
        Get the URL to connect to the sandbox's envd service.

        :param sandbox_id: Sandbox ID
        :param sandbox_domain: Sandbox domain (for cloud deployments)
        :param host_ip: Host IP address (for local deployments, from API response)

        :return: URL to connect to envd
        """
        if self._sandbox_url:
            return self._sandbox_url

        # Local mode: use host_ip from API response or template
        if self.local_mode and host_ip:
            url = self.local_envd_url_template.format(
                host_ip=host_ip,
                port=self.envd_port,
    ...
    def get_host(self, sandbox_id: str, sandbox_domain: str, port: int, host_ip: Optional[str] = None) -> str:
        """
        ...
        :param host_ip: Host IP address (for local deployments)

        :return: Host address to connect to
        """
        # Local mode with host_ip: use the host IP directly
        if self.local_mode and host_ip:
