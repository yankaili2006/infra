...
class SandboxOpts(TypedDict):
    ...
    host_ip: Optional[str]  # Host IP for local deployments
...
class SandboxBase:
    ...
    def __init__(
        ...
        # For local mode, use localhost:49983 as the default envd URL
        # This is because the current local deployment binds all sandboxes to 127.0.0.1:49983
        if connection_config.local_mode:
            # Use host_ip if provided, otherwise default to localhost
            local_host = host_ip or "127.0.0.1"
            self.__envd_api_url = f"http://{local_host}:{connection_config.envd_port}"
        else:
            self.__envd_api_url = self.connection_config.get_sandbox_url(
                self.sandbox_id, self.sandbox_domain, host_ip
            )
    ...
    def host_ip(self) -> Optional[str]:
        """Host IP address for local deployments."""
